...
const ModeratorTeamsTab: React.FC = () => {
        ...
        async (pageNum: number, append: boolean = false) => {
            try {
                ...
                const res = await axiosInstance.get<
                    PageResponse<TeamReportDto>
                >(
                    `/moderator/reportTeams?page=${pageNum}&size=${PAGE_SIZE}`
                );

                const mapped: TeamReportItem[] = res.data.content.map(
                    (item) => ({
                        id: item.id,

                        teamId: item.teamId,
                        teamName: item.teamName,
                        teamAvatarUrl: item.teamAvatarUrl,
                ...
                console.error(e);
                setError("Nie udało się załadować zgłoszonych drużyn.");
            } finally {
                setLoading(false);
    ...
    const handleAccept = async (id: number) => {
        try {
            await axiosInstance.patch(
                `/moderator/reportTeam/${id}/accept`
            );
            setReports((prev) =>
                prev.map((r) =>
            ...
            console.error(e);
            alert("Nie udało się zaakceptować zgłoszenia drużyny.");
        }
    };
    ...
    const handleReject = async (id: number) => {
        try {
            await axiosInstance.patch(
                `/moderator/reportTeam/${id}/reject`
            );
            setReports((prev) =>
                prev.map((r) =>
            ...
            console.error(e);
            alert("Nie udało się odrzucić zgłoszenia drużyny.");
        }
    };

    const handleDelete = async (id: number) => {
        if (!window.confirm("Na pewno usunąć to zgłoszenie drużyny?")) return;

        try {
            await axiosInstance.delete(
                `/moderator/reportTeam/${id}/delete`
            );
            setReports((prev) => prev.filter((r) => r.id !== id));
        } catch (e) {
            console.error(e);
            alert("Nie udało się usunąć zgłoszenia drużyny.");
        }
    };

    const handleToggleViewed = async (report: TeamReportItem) => {
        const { id, viewed } = report;

        try {
            if (!viewed) {
                await axiosInstance.patch(
                    `/moderator/reportTeam/${id}/toggle-viewed`
                );
                setReports((prev) =>
                    prev.map((r) =>
                ...
                await axiosInstance.patch(
                    `/moderator/reportTeam/${id}/toggle-unviewed`
                );
                setReports((prev) =>
                    prev.map((r) =>
            ...
            console.error(e);
            alert("Nie udało się zmienić statusu przeczytania zgłoszenia drużyny.");
        }
    };
            ...
            <div className="overflow-x-auto rounded-2xl border border-zinc-800">
                <table className="min-w-full text-sm">
                    ...
                    <tr className="text-zinc-400">
                        ...
                        <th className="text-left px-4 py-3">Drużyna</th>
                                ...
                                <td className="px-4 py-3 text-right">
                                    <div className="flex items-center justify-end gap-2">
                                        <button
                                            ...
                                            title="Akceptuj zgłoszenie drużyny"
                                        >
                                        ...
                                        <button
                                            ...
                                            title="Odrzuć zgłoszenie drużyny"
                                        >
                                            <X className="h-4 w-4" />
                                        </button>
            ...
            >
                {loadingMore
                    ...
                    : hasNext
                        ...
                        : reports.length > 0
                            ? "Załadowano wszystkie zgłoszenia drużyn."
                            : null}
            </div>
        </section>
    );
...
export default ModeratorTeamsTab;
