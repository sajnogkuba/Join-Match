...
const ModeratorEventRatingsTab: React.FC = () => {
        ...
        async (pageNum: number, append: boolean = false) => {
            try {
                ...
                const res = await axiosInstance.get<
                    PageResponse<EventRatingReportItem>
                >(
    ...
    const handleAccept = async (id: number) => {
        try {
            await axiosInstance.patch(
                `/moderator/reportEventRating/${id}/accept`
            );
            setReports((prev) =>
                prev.map((r) =>
    ...
    const handleReject = async (id: number) => {
        try {
            await axiosInstance.patch(
                `/moderator/reportEventRating/${id}/reject`
            );
            setReports((prev) =>
                prev.map((r) =>
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await axiosInstance.delete(
                `/moderator/reportEventRating/${id}/delete`
            );
    ...
    const handleToggleViewed = async (report: EventRatingReportItem) => {
        const { id, viewed } = report;

        try {
            if (!viewed) {
                await axiosInstance.patch(
                    `/moderator/reportEventRating/${id}/toggle-viewed`
                );
                setReports((prev) =>
                    prev.map((r) =>
                ...
                await axiosInstance.patch(
                    `/moderator/reportEventRating/${id}/toggle-unviewed`
                );
                setReports((prev) =>
                    prev.map((r) =>
...
export default ModeratorEventRatingsTab;
