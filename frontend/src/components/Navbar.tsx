						...
						{navLinks.map(link => (
							<Link
								...
								className={`font-medium transition-colors ${
									location.pathname === link.to ? 'text-violet-400' : 'text-zinc-300 hover:text-white'
								}`}>
								{link.label}
							</Link>
						))}
					...
					<div className='hidden md:flex items-center space-x-4'>
						...
						{!isAuthenticated ? (
							...
							<div className='relative'>
								<button onClick={() => setShowProfileMenu(!showProfileMenu)} className='relative focus:outline-none'>
									<Avatar
