...
interface UniversalEventsCalendarProps {
	events: CalendarEvent[]
}

const UniversalEventsCalendar: React.FC<UniversalEventsCalendarProps> = ({ events }) => {
	const [currentMonth, setCurrentMonth] = useState(dayjs())

	// Funkcja normalizująca datę
	const normalizeDate = (d: number[] | string | Date): Dayjs => {
		if (Array.isArray(d)) {
	...
	const eventsByDate = useMemo(() => {
		...
		events.forEach(event => {
			if (event.eventDate) {
				const dateKey = normalizeDate(event.eventDate).format('YYYY-MM-DD')
											...
											{dayEvents.map(event => (
												<Link
													...
													<div className='text-xs text-zinc-400 mt-1'>
														{normalizeDate(event.eventDate).format('HH:mm')}
													</div>
												</Link>
											))}
										</div>
...
export default UniversalEventsCalendar
